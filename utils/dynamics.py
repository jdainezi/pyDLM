       ...
       #grid = numpy.argwhere(struct_mesh_dict['full_set']['grid_map']==grid)[0][0] +1
        for dof in [2]:
            if ndof == 1:
                dof = 0
           #newK[(grid)*ndof+dof,(grid)*ndof+dof] += ks[i] #+2 para aplicar em Z
            newK[K.shape[0]+i,(grid+ny)*ndof+dof] += ks[i]/(2*dx)  #+2 para aplicar em Z
            newK[K.shape[0]+i,(grid-ny)*ndof+dof] -= ks[i]/(2*dx)  #+2 para aplicar em Z
            newK[(grid+ny)*ndof+dof,K.shape[1]+i] -= ks[i]/(2*dx) #+2 para aplicar em Z
            newK[(grid+ny)*ndof+dof,(grid+ny)*ndof+dof] -= ks[i]/(2*dx)**2 #+2 para aplicar em Z
            newK[(grid+ny)*ndof+dof,(grid-ny)*ndof+dof] += ks[i]/(2*dx)**2 #+2 para aplicar em Z
            newK[(grid-ny)*ndof+dof,K.shape[1]+i] -= ks[i]/(2*dx) #+2 para aplicar em Z
            newK[(grid-ny)*ndof+dof,(grid-ny)*ndof+dof] -= ks[i]/(2*dx)**2 #+2 para aplicar em Z
            newK[(grid-ny)*ndof+dof,(grid+ny)*ndof+dof] += ks[i]/(2*dx)**2 #+2 para aplicar em Z
           ...
           #newM[M.shape[0]+i,(grid-1)*ndof+dof] += ms[i] 
           #newB[(grid)*ndof+dof,(grid)*ndof+dof] += bs[i] #+2 para aplicar em Z
            newB[K.shape[0]+i,(grid+ny)*ndof+dof] += bs[i]/(2*dx) #+2 para aplicar em Z
            newB[K.shape[0]+i,(grid-ny)*ndof+dof] -= bs[i]/(2*dx) #+2 para aplicar em Z
            newB[(grid+ny)*ndof+dof,K.shape[1]+i] -= bs[i]/(2*dx) #+2 para aplicar em Z
            newB[(grid+ny)*ndof+dof,(grid+ny)*ndof+dof] -= bs[i]/(2*dx)**2 #+2 para aplicar em Z
            newB[(grid+ny)*ndof+dof,(grid-ny)*ndof+dof] += bs[i]/(2*dx)**2 #+2 para aplicar em Z
            newB[(grid-ny)*ndof+dof,K.shape[1]+i] -= bs[i]/(2*dx) #+2 para aplicar em Z
            newB[(grid-ny)*ndof+dof,(grid-ny)*ndof+dof] -= bs[i]/(2*dx)**2 #+2 para aplicar em Z
            newB[(grid-ny)*ndof+dof,(grid+ny)*ndof+dof] += bs[i]/(2*dx)**2 #+2 para aplicar em Z
...
def transmissibility(struct_mesh_dict,omegas,grids_ids,ref_grid,ndof,nmodes,phi,omega_n,mi,Mtot,tmode,zeta,zeta_abs,yb,nabs=0,nabst=0,nabsf=0):
    ...
    if ndof == 1:
        if nabst == 0:
            ...
            indf = numpy.hstack([numpy.arange(0,phi.shape[0]-nabst-nabsf),numpy.arange(phi.shape[0]-nabsf,phi.shape[0])])
            indt = numpy.arange(phi.shape[0]-nabsf-nabst,phi.shape[0]-nabsf)
            ybhh += numpy.dot(numpy.linalg.pinv(phi[indt,:]),yb[indt]) 
            ybhh += numpy.dot(numpy.linalg.pinv(phi[indf,:]),yb[indf]) 
    else:
        for dof in [2]:
           ybhh  += numpy.dot(numpy.linalg.pinv(phi[dof::ndof,:]),yb[dof::ndof])
    yb = numpy.dot(phi,ybhh)
...
def aero_transmissibility(GAF,rho,Mhh,Khh,Chh,ks,semi_chord,v,struct_mesh_dict,omegas,grids_ids,ref_grid,ndof,nmodes,phi0,omega_n,mi,Mtot,tmode,zeta,zeta_abs,yb,nabs=0,nabst=0,nabsf=0):
    '''Calculates the transmissibility function using modal approach'''
    frf = []
    mi = numpy.array(mi)
    if nabs != 0:
    ...
    for j,omega in enumerate(omegas):
       ...
       #sys.stdout.flush()
        ...
        if ndof == 1:        
            frf.append(numpy.dot(phi[ref_grid-1,:],eta[:])/numpy.dot(phi,ybhh)[ref_grid-1,0])
        else:
            frf.append(numpy.dot(phi[(ref_grid-1)*ndof+2,:],eta[:])/numpy.dot(phi,ybhh)[(ref_grid-1)*ndof+2,0])
            
    return frf

def aero_transmissibility_old(struct_mesh_dict,GAF,ks,v,rho,phi,semi_chord,Mhh,Chh,Khh,omega_n,omegas,grids_ids,ref_grid,ndof,nmodes,mi,Mtot,tmode,yb,zeta_abs,nabs=0,nabst=0,nabsf=0):
    '''Calculates the transmissibility function using modal approach'''
    frf = []
    mi = numpy.array(mi)
    if nabs != 0:
    ...
    for j,omega in enumerate(omegas):
       ...
       #sys.stdout.flush()
        ...
        if ndof == 1:        
            frf.append(numpy.dot(phinew[ref_grid-1,:],eta[:])/numpy.dot(phinew,ybhh)[ref_grid-1,0])
        else:
            frf.append(numpy.dot(phinew[(ref_grid-1)*ndof+2,:],eta[:])/numpy.dot(phinew,ybhh)[(ref_grid-1)*ndof+2,0])
